...
export function makeDomainsDeleteCommand(globalOpts: () => GlobalOpts): Command<any, any, any> {
    ...
    .action(async (id) => {
      ...
      if (!shouldOutputJson(global)) {
        ...
        const confirmed = await confirm({
          message: `Remove domain ${id}? This cannot be undone.`,
        })
      ...
      process.stdout.write(pc.green('✓') + ` Removed domain ${id}\n`)
    })
}
